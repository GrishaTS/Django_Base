...
class UsersProcessorTests(TestCase):
    ...
    def test_show_users_context(self):
        urls = ['about:description', 'catalog:item_list',
                'feedback:feedback', 'homepage:home',
                'users:user_list', 'users:signup']
        for value in urls:
            with self.subTest("Failed to open url", value=value):
                response = Client().get(reverse(value))
    ...
    def test_show_several_users_context(self):
        Profile.objects.create(email='user2@user.ru',
                               password='smartuser2',
                               birthday=datetime.today())
